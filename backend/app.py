...
def create_todo():
    ...
    new_todo = {
        'id': new_id,
        'todo': data['todo'],
        'isDone': int(data.get('isDone', False)),
        'updated_at': now,
        'deleted': 0
    }
    ch_client.insert('todos_stream', [new_todo])
    # Fetch the inserted todo
    result = ch_client.query(f"SELECT id, todo, isDone, updated_at FROM todos_stream WHERE id = {new_id} AND deleted = 0 ORDER BY updated_at DESC LIMIT 1")
...
def update_todo(todo_id):
    ...
    ch_client.insert('todos_stream', [updated_todo])
    # Fetch updated todo
    result = ch_client.query(f"SELECT id, todo, isDone, updated_at FROM todos_stream WHERE id = {todo_id} AND deleted = 0 ORDER BY updated_at DESC LIMIT 1")
    todo_row = result.result_rows[0]
    todo_dict = dict(zip(result.column_names, todo_row))
...
def delete_todo(todo_id):
    ...
    ch_client.insert('todos_stream', [deleted_todo])
    return jsonify(todo_dict)
...
def toggle_todo(todo_id):
    ...
    toggled_todo = {
        'id': todo_id,
        'todo': todo['todo'],
        'isDone': 0 if todo['isDone'] else 1,
        'updated_at': now,
        'deleted': 0
    }
    ch_client.insert('todos_stream', [toggled_todo])
    # Fetch updated todo
    result = ch_client.query(f"SELECT id, todo, isDone, updated_at FROM todos_stream WHERE id = {todo_id} AND deleted = 0 ORDER BY updated_at DESC LIMIT 1")
    todo_row = result.result_rows[0]
    todo_dict = dict(zip(result.column_names, todo_row))
...
# --- ClickHouse Stream API ---
@app.route('/api/stream_todo', methods=['POST'])
def stream_todo():
    data = request.get_json()
    if not data or 'id' not in data or 'todo' not in data:
        return jsonify({'error': 'Invalid data'}), 400
    ch_client.insert('todos_stream', [{
        'id': data['id'],
        'todo': data['todo'],
        'isDone': int(data.get('isDone', 0))
    }])
    return jsonify({'status': 'inserted'})

@app.route('/api/stream_todos', methods=['GET'])
def get_stream_todos():
    result = ch_client.query('SELECT id, todo, isDone, created_at FROM todos_stream ORDER BY created_at DESC LIMIT 100')
    todos = [dict(zip(result.column_names, row)) for row in result.result_rows]
